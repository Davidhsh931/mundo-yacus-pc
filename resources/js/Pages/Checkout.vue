...
<template>
                            ...
                            <div class="bg-red-50 border border-red-100 rounded-xl p-6">
                                ...
                                <div v-if="cart && Object.keys(cart).length > 0" class="space-y-2">
                                    <div v-for="(item, id) in cart" :key="id" 
                                         class="flex justify-between items-center bg-white p-3 rounded-lg border border-red-100">
                                        <div class="flex items-center gap-3">
                                            <div class="w-8 h-8 bg-red-100 rounded-full flex items-center justify-center text-red-600 font-medium text-sm">
                                                {{ item.quantity }}
                                            </div>
                                ...
                                <div class="border-t border-red-200 mt-4 pt-4">
                                    <div class="flex justify-between items-center">
                                        ...
                                        <span class="text-lg font-medium text-red-700">S/. {{ total.toFixed(2) }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <form @submit.prevent="submit" class="space-y-8">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2 flex items-center gap-2">
                                    <svg class="w-4 h-4 text-red-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round">
                                ...
                                <input 
                                    ...
                                    :class="[
                                        'w-full border bg-gray-50 rounded-lg p-3 focus:ring-red-700 focus:border-red-700 text-gray-900 placeholder-gray-400 transition-colors text-sm',
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-4 text-center flex items-center justify-center gap-2">
                                    <svg class="w-4 h-4 text-red-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                            ...
                            <div class="bg-red-50 border border-red-200 rounded-xl p-6">
                                <div class="flex justify-between items-center mb-4">
                                    ...
                                    <span class="bg-red-600 text-white px-3 py-1 rounded-full text-xs font-medium">SIN COMISIONES</span>
                                </div>
                                <div class="flex justify-between items-center">
                                ...
                                <p class="text-xs mt-3 text-red-700 text-center font-medium">Pago directo al productor de Yacus</p>
                            </div>

                            <!-- Información de Envío -->
                            <div class="bg-red-50 border border-red-200 rounded-xl p-4">
                                <div class="flex items-start gap-3">
                                    <svg class="w-5 h-5 text-red-600 mt-0.5 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                                        <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/>
                                        <circle cx="12" cy="9" r="2.5"/>
                                    </svg>
                                    <div>
                                        <p class="text-sm font-medium text-red-900">Método predeterminado: Recojo en local</p>
                                        <p class="text-xs text-red-700 mt-1">Para envíos a domicilio, coordina el costo adicional por WhatsApp</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <button type="submit" 
                                    ...
                                    class="w-full bg-red-700 text-white py-3 rounded-lg font-medium hover:bg-red-800 transition-colors flex items-center justify-center gap-2">
    ...
    <div v-if="showConfirmModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-3xl max-w-md w-full p-8 shadow-2xl border-2 border-red-500">
            <div class="text-center mb-6">
                ...
                <p class="text-3xl font-black text-red-600 mt-2">S/. {{ total.toFixed(2) }}</p>
            </div>
            ...
            <div class="flex gap-3">
                ...
                <button @click="confirmOrder" 
                        :disabled="form.processing"
                        class="flex-1 bg-red-700 text-white py-3 px-6 rounded-2xl font-black hover:bg-red-800 transition-colors disabled:opacity-50">
