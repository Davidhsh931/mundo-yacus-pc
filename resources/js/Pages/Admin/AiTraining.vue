...
<template>
        ...
        <div class="max-w-7xl mx-auto p-6 sm:p-10 bg-slate-50 min-h-screen font-sans selection:bg-red-100 selection:text-red-700">
            ...
            <header class="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
                ...
                <button 
                    ...
                    :class="showNewCategoryForm ? 'bg-white text-slate-600 border-slate-200 shadow-sm' : 'bg-slate-900 text-white shadow-xl shadow-slate-200 hover:bg-red-600'"
            ...
            <transition enter-active-class="transition duration-300 ease-out" enter-from-class="transform scale-95 opacity-0" enter-to-class="transform scale-100 opacity-100" leave-active-class="transition duration-200 ease-in" leave-from-class="transform scale-100 opacity-100" leave-to-class="transform scale-95 opacity-0">
                <div v-if="showNewCategoryForm" class="bg-white/80 backdrop-blur-md p-8 rounded-3xl border border-white shadow-[0_20px_50px_rgba(220,38,38,0.1)] mb-12 relative overflow-hidden">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div v-for="category in categories" :key="category.id" 
           class="group bg-white rounded-[2rem] shadow-sm border border-slate-200 hover:shadow-2xl hover:shadow-red-500/5 hover:border-red-200 transition-all duration-500 flex flex-col relative overflow-hidden">
        
        <div class="absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-100 transition-opacity">
             <span class="text-4xl font-black text-slate-100 group-hover:text-red-50">#{{ category.id }}</span>
        </div>
          ...
          <div class="flex justify-between items-start">
            ...
            <div class="flex gap-2">
                ...
                <button 
                  ...
                  class="p-2.5 bg-red-50 text-red-500 rounded-xl hover:bg-red-600 hover:text-white transition-all active:scale-90"
